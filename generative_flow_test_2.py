...
class FlowingBrushStrokes:
    """
    Creates continuous flowing brush strokes that flow downward from a mouse-controlled center point.
    ...
    def __init__(self, width=800, height=600):
        ...
        for i in range(self.num_lines):
            # Each line flows downward with slight variations
            # Base angle pointing downward (π/2 = 90 degrees)
            base_angle = math.pi / 2
            angle_variation = random.uniform(-0.6, 0.6)  # More spread for downward flow
            start_angle = base_angle + angle_variation
            
            speed = random.uniform(5.5, 8.5)  # pixels per frame
    ...
    def update(self, frame_time):
        ...
        for line in self.flow_lines:
            ...
            if line['growing'] and line['current_length'] < line['max_length']:
                ...
                current_angle = line['start_angle'] + curve_offset * 0.02
    ...
    def _reset_line(self, line):
        ...
        # Reset to flow downward from current center
        base_angle = math.pi / 2
        angle_variation = random.uniform(-0.6, 0.6)
        line['start_angle'] = base_angle + angle_variation
        line['current_angle'] = base_angle + angle_variation
    ...
    def update_center(self, x, y):
        """Update the center point where lines start flowing from"""
        self.center_x = x
        self.center_y = y
...
#     """Mouse callback to update the center point"""
#     flow_art = param
#     if event == cv2.EVENT_MOUSEMOVE:
#         flow_art.update_center(x, y)
...
def main():
    ...
    # Set mouse callback to track mouse movement
    # cv2.setMouseCallback("Generative Flow Art", mouse_callback, flow_art)
    ...
    # print("Move mouse to control center point")
    # print("Lines flow downward from mouse position")
    print("Lines flow downward from center")
